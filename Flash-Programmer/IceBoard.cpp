...
Result InitBoard()
{
    ...
    deviceStatus = FT4222_SPIMaster_Init(IceBoardHandle, SPI_IO_SINGLE, CLK_DIV_8, CLK_IDLE_HIGH, CLK_TRAILING, 0x01); // TODO: Verify last argument is correct
    if (deviceStatus != FT_OK)
    {
        result = { (int)deviceStatus, resultMessages[deviceStatus] };
        return result;
...
Result PageProgramFlash(int pageIndex, std::vector<uint8> writeBuffer)
{
    ...
    result = WriteSPI({ PageProgramCmd }, 1, false);
    if (result.code)
        return result;

    result = WriteSPI(IntToByteVec(pageIndex), 3, false);
    if (result.code)
        return result;
...
Result ReadPageFlash(int pageIndex, std::vector<uint8>* readBuffer)
{
    ...
    result = WriteSPI({ ReadCmd }, 1, false);
    if (result.code)
        return result;
